...
test("throws \"CRESTError\" when \"response.ok\" is \"false\"", async (t) => {
    ...
    await t.throwsAsync(throwFailedResponse(response, { reason: true }), CRESTError);
});

test("throws \"CRESTError\" with \"url\" from \"response\"", async (t) => {
    const url = faker.internet.url();
    const response = new Response(null, { status: 500, url });
    const error = await t.throwsAsync(throwFailedResponse(response, { reason: true }), CRESTError);
    t.is(error.url, url);
});
...
test("throws \"CRESTError\" with \"statusMessage\" from \"json.reason\"", async (t) => {
    ...
    const response = new Response(null, { status: 500 });
    const error = await t.throwsAsync(throwFailedResponse(response, { reason }), CRESTError);
...
test("throws \"CRESTError\" with \"statusMessage\" from \"response.statusText\" when \"json.reason\" is not present",
    async (t) => {
        ...
        const error = await t.throwsAsync(throwFailedResponse(response), CRESTError);
...
test("throws \"CRESTError\" with \"message\" from \"json.message\"", async (t) => {
    ...
    const response = new Response(null, { status: 500 });
    const error = await t.throwsAsync(throwFailedResponse(response, { message }), CRESTError);
